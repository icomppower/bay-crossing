...
const LICENCE_WORD = { 'sf-buildings.geojson': 'PDDL', 'sausalito-osm.json': 'ODbL', 'terrain-3dep.tif': 'Public domain', 'bathy-ncei.tif': 'Public domain', 'noaa-datums-9414290.json': 'Public domain', 'landmarks-osm.json': 'ODbL', 'noaa-enc-landmarks.json': 'Public domain', 'noaa-enc-pylons.json': 'Public domain', 'ggt-gtfs.zip': 'No licence stated', 'naip-sf.tif': 'Public domain', 'naip-sausalito.tif': 'Public domain', 'naip-bay.tif': 'Public domain' };
...
function check(dir, credits) {
  ...
  for (const [file, box] of [['naip-sf.tif', slice.boxes.sfBuildings], ['naip-sausalito.tif', slice.boxes.sausalito], ['naip-bay.tif', NAIP_BAY]]) {
    const t = readTiff(readFileSync(join(dir, file))), b = box.minE !== undefined ? box : naipBox(box), cell = box.cell || 1;
...
if (!process.argv.includes('--negative')) {
  ...
  console.log('G0 PASS — 12 sources cached, checksummed, plausible, licences recorded');
  process.exit(0);
}
...
console.log(`NEGATIVE ${Object.keys(MUTATIONS).length - uncaught}/${Object.keys(MUTATIONS).length}`); // verify.sh requires every mutation caught
