...
export function writeTiles(merged, outDir) {
  const { res, tile, size } = GRID, n = res / tile;
  mkdirSync(outDir, { recursive: true });
  for (const f of readdirSync(outDir)) if (/^t_\d+_\d+\.bin$/.test(f) || f === 'index.json') rmSync(join(outDir, f));
  const files = [];
...
if (process.argv[1] === fileURLToPath(import.meta.url)) {
  ...
  const index = writeTiles(merged, arg('--out', join(root, 'public/terrain')));
